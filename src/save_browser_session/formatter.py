...
def generate_csv(
    ...
    writer.writerow(
        ["Window ID", "Tab ID", "Group", "Title", "URL", "Session", "Timestamp"]
    )
...
<head>
    ...
    <style>
        ...
        h3 {{ font-size: 15px; color: #444; margin: 16px 0 6px 0; border-bottom: 1px dashed #e5e5e5; padding-bottom: 4px; }}
...
def generate_html(
    ...
    body_html = ""
    for i, w in enumerate(windows):
        body_html += f'<div class="window"><h2>Window {i + 1} ({len(w.tabs)} tabs)</h2>'
        has_groups = any(t.group for t in w.tabs)
        if not has_groups:
            body_html += "<ul>"
            for t in w.tabs:
            ...
            for group_name, group_tabs_iter in groupby(w.tabs, key=lambda t: t.group):
                group_tabs = list(group_tabs_iter)
                group_title = html.escape(
                    group_name if group_name else "Ungrouped Tabs"
                )
                body_html += f"<h3>{group_title} ({len(group_tabs)} tabs)</h3><ul>"
                for t in group_tabs:
                    title = html.escape(t.title or t.url)
                    url_display = html.escape(t.url)
                    url_href = html.escape(t.url, quote=True)
...
def generate_formatted_content(
    ...
    for win in windows:
        ...
        for tab in win.tabs:
            ...
            valid_tabs.append(Tab(clean_t, clean_u, id=tab.id, group=tab.group))
        ...
        for i, win in enumerate(filtered_windows):
            ...
            if not has_groups:
                ...
                for group_name, group_tabs_iter in groupby(
                    ...
                    body += f"\n### {group_header} ({len(group_tabs)} Tabs)\n\n"

                    for vt in group_tabs:
                        tab_global_idx += 1
                        num_str = f"{tab_global_idx}."
                        padded_num = f"{num_str:<{max_width}}"
